...
public class LunarCalendar {
    ...
    final static String[] chineseNumber = {"一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二", "零"};
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < 32; i++) {
            String lunarDate1 = lunarCalendar.getLunarDate(2020, 1, i, false);
            String lunarDate = lunarCalendar.getLunarDate(2020, 1, i, true);
            String format = String.format("公历：%s年%s月%s日 农历：%s %s年 %s年%s%s %s",
                    2020, 1, i,
                    lunarCalendar.cyclical(lunarCalendar.year), lunarCalendar.animalsYear(lunarCalendar.year),
                    lunarCalendar.getCYear(), lunarCalendar.getLunarMonth(), lunarDate, lunarDate1);
    ...
    /**
     * 天干：年份减3，除以10，没有余数就是天干的最后一个，余数是1对应 甲 ，是2对应乙，依次往后推
    ...
    public String getCYear() {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (j == 0){
                yearStr.append(chineseNumber[chineseNumber.length - 1]);
            }else{
                yearStr.append(chineseNumber[j - 1]);
            }
        }
